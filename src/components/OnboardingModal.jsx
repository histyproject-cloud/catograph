...
function RelationshipVisual() {
  const nodes = [
    { label: "에쉬", role: "남주1", x: 25, y: 20, color: "#f59e0b" },
    { label: "지니", role: "주인공", x: 65, y: 55, color: "#34d399" },
    { label: "제카르", role: "남주2", x: 20, y: 68, color: "#818cf8" },
  ];
  const edges = [
...
function WorldVisual() {
  const links = [
    { type: "그림", title: "제카르 팬아트 모음", color: "#818cf8" },
    { type: "소설", title: "만약 배경이 아카데미라면?", color: "#34d399" },
    { type: "그림", title: "현대AU", color: "#818cf8" },
  ];
  return (
    <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 8 }}>
      {links.map((item, i) => (
